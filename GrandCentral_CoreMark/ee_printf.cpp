...
/* newlib on this target does not provide ecvtbuf/fcvtbuf, so implement
   minimal versions sufficient for printing benchmark scores (%e/%f/%g). */
char *
fcvtbuf(double arg, int ndigits, int *decpt, int *sign, char *buf)
{
    unsigned long long ival, scale = 1;
    char               tmp[32];
    int                len = 0, i;
    ...
    *sign = (arg < 0) ? 1 : 0;
    ...
    for (i = 0; i < ndigits; i++)
        scale *= 10;
    ival = (unsigned long long)(arg * (double)scale + 0.5);
...
{
    unsigned long long ival, scale = 1, maxval = 1;
    char               tmp[32];
    int                len = 0, i, exp10 = 0;
